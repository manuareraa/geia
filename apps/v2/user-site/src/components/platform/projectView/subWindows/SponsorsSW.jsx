...
function SponsorsSW(props) {
  ...
  const renderSponsorCards = () => {
    ...
    props.sponsorsData.forEach((sponsor, index) => {
      let card = (
        <div
          className="flex flex-col items-center justify-center px-0 py-4 space-y-4 bg-white rounded-xl"
          key={index}
        >
          <img
            src={sponsor.logoUrl}
          ...
          <p
            ...
            onClick={() => {
              window.open(sponsor.linkToSite, "_blank");
            }}
          >
            {sponsor.sponsorName}
          </p>
      ...
      <div className="flex flex-col px-6 pb-16 mt-20 lg:px-60">
        <div className="flex flex-col p-8 rounded-xl bg-glGreen">
          ...
          {sponsorCards.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-y-6 lg:gap-y-0 gap-x-4">
              {sponsorCards}
            </div>
          ) : (
